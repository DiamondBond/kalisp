...
typedef struct lval {
  ...
  char *err;
  char *sym;
  /* Count & Pointer to a list of "lval*" */
  int count;
  struct lval **cell;
} lval;

/* Construct a pointer to a Number lval */
...
/* Construct a pointer to a Symbol lval */
...
/* Construct a pointer to a Sexpr lval */
...
/* Construct a pointer to a Qexpr lval */
...
lval *builtin_head(lval *a) {
  ...
  /* Delete all elements that are not head & return */
  while (v->count > 1) {
...
lval *builtin_tail(lval *a) {
  ...
  if (a->cell[0]->count == 0) {
	lval_del(a);
	return lval_err("Function 'tailed' passed {}!");
  }

  /* Take first argument */
  lval *v = lval_take(a, 0);

  /* Delete first element & return */
...
lval *builtin_op(lval *a, char *op) {
  ...
  for (int i = 0; i < a->count; i++) {
	if (a->cell[i]->type != LVAL_NUM) {
	  lval_del(a);
	  return lval_err("Canot operate on a non-number!");
	}
  }
